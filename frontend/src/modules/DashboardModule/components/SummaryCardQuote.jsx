...
export default function QuoteSummary({
  ...
  const fetchQuoteSummary = async (month) => {
    try {
      setLoading(true);
      const response = await axios.get('/quote/summary', {
        params: { type: 'month', month: month ? month.format('YYYY-MM') : null },
      });
      setSummaryData(response.data.result);
    } catch (error) {
      console.error('Error fetching quote summary:', error.response);
      // Display specific error to the user
      if (error.response && error.response.data) {
        alert(error.response.data.message); // Or display in a more user-friendly way
      } else {
        alert('An error occurred while fetching quotes summary.');
      }
    } finally {
      setLoading(false);
      console.log('Quote summary data:', summaryData);
    }
  };

  useEffect(() => {
    fetchQuoteSummary(selectedMonth);
  }, [selectedMonth]);
  const handleMonthChange = (date) => {
    setSelectedMonth(date);
    fetchQuoteSummary(date);
          ...
          <Row gutter={[0, 0]} justify="space-between" wrap={false}>
            ...
            >
              {loading ? (
                ...
                <Tooltip
                  title={
                    summaryData && summaryData.total !== undefined && summaryData.total !== null
                      ? formatEuro(summaryData.total)
                      : ''
                  }
                >
                  <Tag
